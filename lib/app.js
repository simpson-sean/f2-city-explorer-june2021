...
const { mungeWeatherData } = require('./Utils/munge.js');
...
app.get('/weather', async(req, res) => {
  ...
  try {
    ...
    const city = req.query.search;
    const weatherData = await request.get(`https://api.weatherbit.io/v2.0/forecast/daily?city=${city}&key=${process.env.WEATHER}`);
    const data = [
